...
void Reader::ReadfromFile(const string& path ) const
{
    ...
    if (fin.is_open())
    {
        ...
        bar->setStyleSheet("border: 1px green");
        bar->showMessage("file is open!");
        ...
        if (fout.is_open())
        {
            ...
            bar->setStyleSheet("border: 1px solid green");
            bar->showMessage("output file is done!");
        ...
        else
        {
            ...
            bar->setStyleSheet("border: 1px solid red");
            bar->showMessage("file isn't open!");
        }
        fout.close();
    }
    else
    {
        ...
        bar->setStyleSheet("border: 1px solid red");
        bar->showMessage("File for read isn't open!");
    }
    fin.close();
}
